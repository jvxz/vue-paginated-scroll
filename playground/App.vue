...
import { FakeTimeline, type Message } from './data'
...
const lastId = computed(() => events.value[events.value.length - 1]?.id ?? timeline.oldestId)
...
const { window, vItem, isAtLiveEdge, isPaginating, scrollToEdge, debugState } = usePaginatedScroll(
  container,
  {
    source: events,
    getKey: (e) => e.id,
...
<template>
  <div class="app">
    <header>
      ...
      <div class="controls">
        <label>target×<input v-model.number="targetHeight" type="range" min="1.5" max="6" step="0.5" />{{ targetHeight }}</label>
      ...
      <div class="status">
        window: {{ window.length }} rows ·
        ids {{ window[0]?.id ?? '—' }}–{{ window[window.length - 1]?.id ?? '—' }} ·
        liveEdge: {{ isAtLiveEdge ? 'yes' : 'no' }} ·
        paginating: {{ isPaginating.backward ? '↑' : '' }}{{ isPaginating.forward ? '↓' : '' }}{{ !isPaginating.backward && !isPaginating.forward ? 'idle' : '' }}
      </div>
    </header>

    <div ref="container" class="timeline">
      <article
        v-for="event in window"
        :key="event.id"
        v-item="event.id"
        class="msg"
        :class="'msg--' + event.kind"
      >
        <div class="msg__meta">#{{ event.id }} · {{ event.author }} · {{ event.kind }}</div>
        <div class="msg__text">{{ event.text }}</div>
        <div
          v-if="event.blockHeight"
          class="msg__block"
          :style="{ height: event.blockHeight + 'px' }"
        >
...
.controls { display: flex; gap: 14px; align-items: center; flex-wrap: wrap; font-size: 12px; }
.controls label { display: flex; gap: 4px; align-items: center; }
.controls button { font-size: 12px; padding: 3px 8px; border-radius: 5px; border: 1px solid #475569; background: #334155; color: #e2e8f0; cursor: pointer; }
.controls button:hover { background: #475569; }
.status { margin-top: 6px; font-size: 11px; color: #94a3b8; font-family: ui-monospace, monospace; }
.timeline { position: relative; flex: 1; min-height: 600px; overflow-y: auto; padding: 12px; }
.msg { padding: 8px 12px; margin: 6px 0; border-radius: 8px; background: #1e293b; border: 1px solid #263449; }
.msg--image { border-color: #3b4d66; }
.msg--rules { border-color: #5b4a2f; background: #241f16; }
.msg__meta { font-size: 10px; color: #64748b; font-family: ui-monospace, monospace; margin-bottom: 4px; }
.msg__text { font-size: 13px; line-height: 1.45; }
.msg__block { margin-top: 8px; border-radius: 6px; background: #0b1220; display: flex; align-items: center; justify-content: center; color: #64748b; font-size: 12px; border: 1px dashed #334155; }
</style>
